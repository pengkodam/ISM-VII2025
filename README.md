...
This plenary speech demonstrates  a multi-agent framework applied to income prediction using simulated Public Use Microdata Sample (PUMS) data how human statistical oversight transforms AI from computational tool to rigorous scientific instrument. The framework integrates four key components:a Statistical Feedback Agent for bias detection and assumption validation, a Bayesian Inference Agent for predictive modeling, a Conformal Prediction Agent generating distribution-free prediction intervals with guaranteed coverage,  and critically, human decision points where statisticians provide irreplaceable judgment on bias handling and uncertainty interpretation.
